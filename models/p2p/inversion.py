...
from models.p2p.attention_control import register_attention_control,set_invert_note,clear_invert_note,clear_note
from models.p2p.attention_control import EmptyControl, AttentionStore, make_controller,MyEmptyControl
...
from models.masactrl.masactrl_utils import set_invert_note as set_invert_note_masa
from models.masactrl.masactrl_utils import clear_invert_note as clear_invert_note_masa
...
class NegativePromptInversion:
    ...
    def invert(self, image_gt, prompt, npi_interp=0.0,controller=None):
        """
...
class NullInversion:
    ...
    def ddim_loop(self, latent):
        ...
        return all_latent,step_list


    @torch.no_grad()
    def ddim_loop_uam(self, latent):
        uncond_embeddings, cond_embeddings = self.context.chunk(2)
    ...
    def ddim_inversion(self, image):
        latent = image2latent(self.model.vae, image)
        image_rec = latent2image(self.model.vae, latent)[0]
        ddim_latents,step_list = self.ddim_loop(latent)
        return image_rec, ddim_latents,step_list


    @torch.no_grad()
    def ddim_inversion_uam(self, image):
        latent = image2latent(self.model.vae, image)
        image_rec = latent2image(self.model.vae, latent)[0]
        ddim_latents,step_list = self.ddim_loop_uam(latent)
        return image_rec, ddim_latents,step_list
    ...
    def invert(self, image_gt, prompt, guidance_scale, num_inner_steps=10, early_stop_epsilon=1e-5,controller=None):
        self.init_prompt(prompt)
        
        clear_invert_note()
        clear_note()
        register_attention_control(self.model, None)
        image_rec, ddim_latents,step_list = self.ddim_inversion(image_gt)
        ...
        set_invert_note()
        image_rec_uam, ddim_latents_uam,step_list_uam = self.ddim_inversion_uam(image_gt)
        clear_invert_note()

        return image_gt, image_rec, ddim_latents, uncond_embeddings,step_list,image_rec_uam, ddim_latents_uam,step_list_uam
...
class DirectInversion:
    ...
    def ddim_loop_uam(self, latent):
        ...
        cond_embeddings=uncond_embeddings[[0]] 
        all_latent = [latent]
        latent = latent.clone().detach()
    ...
    def ddim_inversion_uam(self, image):
        latent = image2latent(self.model.vae, image)
        image_rec = latent2image(self.model.vae, latent)[0]
        ddim_latents = self.ddim_loop_uam(latent)
        return image_rec, ddim_latents
    ...
    def invert(self, image_gt, prompt, guidance_scale, num_inner_steps=10, early_stop_epsilon=1e-5):
        ...
        image_rec_uam, ddim_latents_uam = self.ddim_inversion_uam(image_gt)

        return image_gt, image_rec, ddim_latents, noise_loss_list,image_rec_uam, ddim_latents_uam
